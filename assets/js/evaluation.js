(function(global){
  ...
  VA.calcExerciseScoreForStudent = function(stu,exIdx){
    ...
    (ex.covers||[]).forEach(function(indId){ const level=getSavedLevel(stu,ex.id,indId); const cell=calcCellScore(indId,level);
      const cov=VA.coverageCountForIndicatorById(indId)||1; sum+=(cell/cov); });
  ...
  VA.calcIndicatorMeanForStudent = function(stu,indId){
    ...
    let sum=0; idxs.forEach(function(exIdx){ const ex=VA.app.exercises[exIdx]; const level=getSavedLevel(stu,ex.id,indId); sum+=calcCellScore(indId,level); });
  ...
  function buildStudentTableHtml(student){
    ...
    VA.app.indicators.forEach(function(ind){
      ...
      tbody += '<td class="text-center"><span class="badge text-bg-light" data-ind-mean="'+VA.escapeHtml(student)+'::'+ind.id+'">'+mean.toFixed(2)+'</span> / <span class="badge text-bg-light">'+(max.toFixed ? max.toFixed(1) : max)+'</span></td>';
      tbody += "</tr>";
    });
